...
class ArduinoMotor(QtCore.QThread):
    name = 'arduino_Motors'

    def __init__(self, port):
        self.port = port
        super(ArduinoMotor, self).__init__()
        ...
        self.parameter_display_dict['Motor_1']['val'] = 0
        self.parameter_display_dict['Motor_1']['unit'] = ' mm'
        self.parameter_display_dict['Motor_1']['max'] = 300
        self.parameter_display_dict['Motor_1']['min'] = -300
        self.parameter_display_dict['Motor_1']['read'] = False
    ...
    def set_parameter(self, parameter, value):
        if parameter == 'Motor_1':
            change_in_mm = self.parameter_dict['Motor_1'] -value
    ...
    def update_shutter(self,shutter, set_angle):
        ...
        command = 'MTR' + str(shutter) + '=' + str(set_angle)
        try:
            self.ser.write(command.encode())
        except serial.SerialTimeoutException:
            print(time.strftime('%H:%M:%S') + 'Arduino serial timeout exception')
