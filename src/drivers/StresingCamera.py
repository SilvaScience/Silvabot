...
class StresingCamera(QtCore.QThread):
    ...
    def __init__(self,hardware_params):
        ...
        # stresing/ESLSCDLL.dll: this driver needs ESLSCDLL 4.17.8.0, whose API dropped
        # DLLInitBoard, DLLSetGlobalSettings and DLLReturnFrame. stresing_camera.py (the Wake
        # Forest driver) and samples/drivers/test_streisingCAM.py still call all three, so the
        # older DLL has to stay where it is for them.
        folder_path_dll = Path(__file__).resolve().parent #add or remove parent based on the file location
        path_dll = folder_path_dll / "stresing" / "v4" / "ESLSCDLL.dll"
