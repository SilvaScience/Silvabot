...
# %%
# input 
date = '2025-10-18' # '2025-07-24' # 
data_name =  'GaAs_QW_2501_17_07_00' # 'GaAs_QW_2501_16_08_00' #
...
with h5py.File(os.path.join(path,date,bg_name + '.h5'), 'r') as f:
    bg_rawI_row = np.flip(np.sum(f['averaged_rawI'][264:270,:],axis =0)) # 264:268
    bg_rawQ_row = np.flip(np.sum(f['averaged_rawQ'][264:270,:],axis =0))
...
for i, filename in enumerate(file_list):
    ...
    with h5py.File(filepath, 'r') as f:
        rawI_row = np.sum(f['rawI'][:, 264:270, :],axis =1)  # shape: (n_avg, x) # 264:268
        rawQ_row = np.sum(f['rawQ'][:, 264:270, :],axis =1) 
...
#filepath = os.path.join(path,date,data_name + '.h5')
with h5py.File(filepath, 'r') as f:
    rawI_row = np.flip(np.sum(f['rawI'][:, 264:270, :],axis =1))  # shape: (n_avg, x)
    rawQ_row = np.flip(np.sum(f['rawQ'][:, 264:270, :],axis =1)) 
    #amp_row = np.sqrt((rawI_row-bg_rawI_row) ** 2 + (rawQ_row-bg_rawQ_row) ** 2)
    raw_image = f['rawQ'][5,:,:]
...
pc = plts[2].pcolor(approx_x_axis,time_axis,rawQ_row)
pc.set_clim(-1,1)
plts[2].set_xlabel('Energy (meV)')
...
if plot_external: plts[0].set_title(f'External reference with RecordingStart trigger  \n {title_id}')
