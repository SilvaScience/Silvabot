...
# %%
# input 
date = '2025-09-22' # '2025-07-24' # 
data_name =  'GaAs_QW_2501_15_43_55' # 'GaAs_QW_2501_16_08_00' #
...
# filter FT spectrum
crit_min = find_idx(freqs_crop,0.020) #0.006  #0.06 #0.18
crit_max = find_idx(freqs_crop,0.022) #0.015 #0.075 #0.195
...
approx_x_axis = np.linspace(1476,1476+150,np.shape(amp_map)[1])
file_idx = 3
data_name = 'GaAs_QW_2501_17_17_27'
title_id = os.path.join(date,data_name)
...
plts[0].set_xticks([])
plts[1].set_xticks([])
plts[0].set_title(f'Raw data at t=0ps Nperiod=100 \n {title_id}')
...
if plot_evolution:
    ...
    plts[0].set_title(f'Raw data at t=2ps Nperiod=100 \n {title_id}')
...
with h5py.File(filepath, 'r') as f:
    ...
    rawQ_row = rawQ_row-np.average(rawQ_row,axis=0)
    amp_row = np.sqrt(rawI_row ** 2 + rawQ_row ** 2)
    amp_row_averagedIQ = np.sqrt(np.average(rawI_row,axis =0) ** 2 + np.average(rawQ_row,axis =0) ** 2)
    phase_row = np.arctan2(rawQ_row,rawI_row)
    #amp_row = amp_row[np.max(amp_row,axis=1)>100]
    mean_amp_row = np.mean(amp_row, axis=0)

f, plts = plt.subplots(3, figsize =(4,8),gridspec_kw={'height_ratios': [1, 1.1,2]})
plts[0].plot(approx_x_axis,np.average(amp_row_averagedIQ,axis=0))
#plts[0].plot(approx_x_axis,np.average(amp_row/np.cos(phase_row),axis=0))
