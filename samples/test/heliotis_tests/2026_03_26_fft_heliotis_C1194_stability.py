...
# %%
# input 
date = '2026-03-26' # '2025-07-24' # 
data_name =  'C1194' # 'GaAs_QW_2501_16_08_00' #
bg_name =  'avg_data10_39_51' # 'avg_data17_11_58' # # load background  ;  CHANGE THIS IF TAU CHANGES THE 
path = r"D:\DATA\BIGFOOT"
title_id = os.path.join(date,data_name)


with h5py.File(os.path.join(path,date,'raw',bg_name + '.h5'), 'r') as f:
    bg_rawI_row = np.flip(np.sum(f['averaged_rawI'][315:320,:],axis =0))
    bg_rawQ_row = np.flip(np.sum(f['averaged_rawQ'][315:320,:],axis =0))
...
filename = 'raw_data15_47_23.h5'
filepath = os.path.join(path,date,'raw', filename)

with h5py.File(filepath, 'r') as f:
    rawI_row = np.sum(f['rawI'][:, 315:320, :],axis =1)  # shape: (n_avg, x)
    rawQ_row = np.sum(f['rawQ'][:, 315:320, :],axis =1) 
    
rawI_row = rawI_row - bg_rawI_row
...
approx_x_axis = np.linspace(1525,1725,np.shape(amp_row)[1])
N_period = 10
freq = 7800
fps = freq/N_period
lim1 = find_idx(approx_x_axis, 1628)
lim2 = find_idx(approx_x_axis, 1632)
...
plts[0].set_xticks([])
plts[1].set_xticks([])
plts[0].set_title(f'Demodulation 7.8kHz Nperiod={N_period} \n {title_id}')

f.colorbar(pc, orientation='horizontal', pad =0.2, shrink=0.9)
#for i in range(3): plts[i].set_xlim(1600,1650)  
...
if plot_evolution:
    ...
    plts[0].plot(time_axis,np.average(rawI_row[:,lim1:lim2],1),label ='rawI')
    plts[1].plot(time_axis,np.average(rawQ_row[:,lim1:lim2],1),label ='rawQ')
    plts[2].plot(time_axis,np.average(amp_row[:,lim1:lim2],1),label ='$(Q^2+I^2)^{(1/2)}$')
    plts[3].plot(time_axis,np.average(clean_phase[:,lim1:lim2],1),label ='$arctan2(Q,I)$')
    ...
    plts[0].set_title(f'Avg. Int. Nperiod={N_period} \n {title_id}')
    
    f.subplots_adjust(wspace =0,hspace =0)
    plt.show()

# %%
plt.hist(np.average(amp_row[:,lim1:lim2],1),bins=30)
# %%
lim1 = find_idx(approx_x_axis, 1621)
lim2 = find_idx(approx_x_axis, 1625)

print(np.mean(np.average(amp_row[:,lim1:lim2],1)))
# %%
