...
# %%
# input 
date = '2026-01-27' # '2025-07-24' # 
data_name =  'raw_data16_45_50' # 'GaAs_QW_2501_16_08_00' #
bg_name =  'avg_data11_13_25' # 'avg_data17_11_58' # # load background  ;  CHANGE THIS IF TAU CHANGES THE 
...
Z = np.transpose(np.flipud(mean_amp))-6.3
#log_norm = colors.LogNorm(vmin=2, vmax=np.nanmax(Z))
plt.pcolor(pixel,approx_x_axis,Z)
plt.clim(0,2)
plt.title(f'{title_id}')
...
pixel = np.r_[540-200:540-140]
plt.plot(approx_x_axis,np.mean(mean_amp[pixel,:],axis=0))
plt.title(f'{title_id}')
plt.xlabel('Energy (meV)')
plt.ylabel('Pixel')
#plt.xlim(1595,1609)
